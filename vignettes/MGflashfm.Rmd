...
1. Within each population group, the trait GWAS must be harmonised so that each variant has the same effect allele across the GWAS. This can be done using the harmoniseGWAS function; an example of its use is in the [flashfmZero](https://jennasimit.github.io/flashfmZero/) package, specifically [here](https://jennasimit.github.io/flashfmZero/articles/Example_Part2.html#harmonise-gwas-data-from-observed-traits).

2. If using meta-analysis summary statistics, only variants that have measurements for at least 80% (for example) of the individuals should be included. This could be carried out be providing a column of sample sizes for each variant in the GWAS data provided to the harmoniseGWAS function. The minimum proportion is specified by the argument minNprop, which is 0.80 by default. If the N column is not specified or not provided, then this filtering will not be done.

3. The trait GWAS must be aligned with the variants of the LD panel. This can be done using the alignGWAS function; an example of its use is in the [flashfmZero](https://jennasimit.github.io/flashfmZero/) package, specifically [here](https://jennasimit.github.io/flashfmZero/articles/Example_Part2.html#align-gwas-to-ld).

4. The MGflashfm, MGfm, and flashfm fine-mapping functions that make use of JAMdynamic include a tagging argument, r2, that is 0.80 by default. This can be reduced for larger regions (e.g. r2=0.6) to increase speed. 
...
#' Set NCORES=no. of groups, if on Linux. Otherwise, set NCORES=1.
...
mgmtCS <- MGFLASHFMwithJAM(gwas.list=gwas.list, LD.list=corX.list, covY.list=covY.list, Nall=Nall, multi=TRUE, TOdds=1, maxcv=1, 
	maxcv_stop = 20, save.path=save.path, cpp=0.99, cred=0.99,NCORES=1,jam.nM.iter=1,r2=.99,flashfmRET=TRUE, extra.java.arguments=NULL) 
``` 
For this small region with in-sample LD we set jam.nM.iter=1 and r2=.99. For larger regions the r2 value could be set lower and is 0.80 by default; at this lower r2 setting we recommend jam.nM.iter=5 (default).
...
More specifically, the above arguments are:
	...
	-  jam.nM.iter: in millions, number of iterations to use in JAM; default 5 (i.e.: 5 million). 
	
	- r2: r.squared threshold for to find tag SNPs to reduce model search space before expanding to all SNPs; default 0.8
...
mgCS <- MGflashfmRET(gwas.list=gwas.list,flashfm.list=flashfm.list,Nall,cred=0.99,multi=TRUE,cpp=0.99,NCORES=1) 
#' here multi=TRUE indicates multi-trait muti-group results are calculated
```
...
```{r}
#' Set multi=TRUE for multi-trait and multi=FALSE for single-trait
#' If using non-Linux machine, set NCORES=1 
mgCS <- MGflashfmRET(gwas.list=gwas.list,flashfm.list=mgmtCS$flashfm.out,Nall=Nall,cred=0.99,multi=FALSE,cpp=0.99,NCORES=1) 
#' here multi=FALSE indicates single-trait multi-group results are calculated. This is included only for comparison purposes and 
#' we recommend multi=TRUE for more power 
```

Look at the resulting 99% credible sets (CS99) from single-trait multi-ancestry fine-mapping and the causal variants to see that the causal variants are contained in the CS99 and have mgMPP > 0.8:
...
This analysis is for the situation where there is one trait and multiple population groups. When multiple traits are available
we recommend MGflashfm.
...
mgCS1 <- MGFMwithJAM(gwas.list=T1gwas.list, corX.list=corX.list,  Nall=N1all, save.path="tmpDIR", cpp=0.99, cred=0.99, maxcv=1, maxcv_stop = 20, NCORES=1, jam.nM.iter=1, r2=0.99, extra.java.arguments=NULL)
``` 

The output from mgCS1 is identical to the Trait 1 results in mgCS, as these are multi-group single-trait fine-mapping results (multi=FALSE in MGflashfm), so we do not save them separately. 
The difference between running MGFLASHFMwithJAM with multi=FALSE and MGFMwithJAM is that MGFMwithJAM is used for only a single trait, using all variants available in the trait GWAS, 
while MGFLASHFMwithJAM with multi=FALSE will run MGfm on each trait using the same variants that would be used in multi-trait fine-mapping (i.e. only variants present in all trait GWAS within each population group, allowing for different variants across population groups). 
...
g2t1CS <- JAMdynamic(gwas=gwas.list[[2]][[1]],corX=corX.list[[2]], ybar=0, Vy=1, N=Nall[[2]][1], cred=.99, save.path="tmpDIR", 
	maxcv=1, maxcv_stop = 20, jam.nM.iter=1, r2=0.99, extra.java.arguments=NULL)
``` 
For this small region with in-sample LD we set jam.nM.iter=1 and r2=.99. For larger regions the r2 value could be set lower and is 0.80 by default; at this lower r2 setting we recommend jam.nM.iter=5 (default).
...
The arguments above are as follows:
	...
	- r2: r.squared threshold for to find tag SNPs to reduce model search space before expanding to all SNPs;
...
It requires: 
	...
	- jam.nM.iter: the number of iterations (in millions) for JAM, the default is 5 (i.e.: 5 Million)
...
mtg1 <- FLASHFMwithJAMd(gwas.list=gwas.list[[1]], corX=corX.list[[1]], ybar=c(0,0), N=Nall[[1]], save.path="tmpDIR", TOdds = 1, covY=covY.list[[1]], cpp = 0.99, NCORES=1, maxcv=1, maxcv_stop = 20,jam.nM.iter=1, r2=0.99, extra.java.arguments=NULL)  
mtg1CS <- allcredsetsPP(mtg1$mpp.pp,cred=.99)      
#' Group 2 multi-trait fine-mapping with flashfm 
 mtg2 <- FLASHFMwithJAMd(gwas.list=gwas.list[[2]], corX=corX.list[[2]], ybar=c(0,0), N=Nall[[2]], save.path="tmpDIR", TOdds = 1, covY=covY.list[[2]], cpp = 0.99, NCORES=1, maxcv=1, maxcv_stop = 20,jam.nM.iter=1, r2=0.99, extra.java.arguments=NULL)   
